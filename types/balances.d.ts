export default Balance;
export type Response = import("../types/apiResponses").balancesResponse;
/**
 * @typedef { import("../types/apiResponses").balancesResponse } Response
 */
/**
 * A Balance represents the financial status of an Account at a given time.
 * @extends Resource<Response>
 */
declare class Balance extends Resource<import("../types/apiResponses").balancesResponse> {
    constructor(session: any);
    /**
     * Retrieve balances from a specific account or all accounts from a specific link.
     * @async
     * @param {string} link - UUID4 representation of a link Id.
     * @param {string} dateFrom - Required date from, format is YYYY-MM-DD
     * @param {object} [options] - Optional parameters (account, dateTo, token, saveData)
     * @param {string} [options.token] - The OTP token generated by the bank.
     * @param {string|null} [options.account] - Only balances matching this account.id are returned.
     * @param {string} [options.dateTo] - Date that you want to stop receiving balances.
     * @param {boolean} [options.saveData] - Indicates whether or not to persist the data in Belvo.
     * @returns {Promise<Response>} Response
     * @throws {RequestError}
     */
    retrieve(link: string, dateFrom: string, options?: {
        token?: string;
        account?: string | null;
        dateTo?: string;
        saveData?: boolean;
    }): Promise<Response>;
    #private;
}
import Resource from "./resources";
